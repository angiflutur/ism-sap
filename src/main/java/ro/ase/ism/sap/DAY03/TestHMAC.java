...
public class TestHMAC {
    public static byte[] getHMAC(String fileName, String algorithm, String password)
        ...
        fis.close(); //it closes the bis too
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException, IOException, InvalidKeyException, InvalidKeySpecException {
        //test hmac
        byte[] hmac = getHMAC("msg.txt", "HmacSHA1", "ism1234");
        System.out.println("HMAC: " + Utility.getHex(hmac));

        hmac = getHMAC("msg.txt", "HmacSHA1", "ism12345");
        System.out.println("HMAC: " + Utility.getHex(hmac));

        //test pbkdf
        byte[] key = getPBKDF("ism1234", "PBKDF2WithHmacSHA1", "rd@h1", 1000, 160);
        System.out.println("Value stored in DB: " + Utility.getHex(key));

        key = getPBKDF("ism1234", "PBKDF2WithHmacSHA1", "dfhfghj", 1000, 160);
        System.out.println("Value stored in DB: " + Utility.getHex(key));

        //benchmark PBKDF performance
        int noIterations = (int)(3*1e5);
        double tStart = System.currentTimeMillis();
        key = getPBKDF("ism1234", "PBKDF2WithHmacSHA1", "dfhfghj", noIterations, 160);
        System.out.println("Value stored in DB: " + Utility.getHex(key));
        double tFinal = System.currentTimeMillis();
        System.out.printf("PBKDF with %d iterations computed in %f seconds.", noIterations, (tFinal - tStart)/1000);
    }
}
